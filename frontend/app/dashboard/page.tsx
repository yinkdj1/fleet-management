...
export default function DashboardPage() {
  ...
  const user = useSyncExternalStore(subscribeToNothing, readStoredUser, () => null);
  ...
  const [discountForm, setDiscountForm] = useState({
    ...
    depositAmount: DEFAULT_DISCOUNT_SETTINGS.depositAmount,
    taxPercentage: DEFAULT_DISCOUNT_SETTINGS.taxPercentage,
    servicePlatformFeePerDay: DEFAULT_DISCOUNT_SETTINGS.servicePlatformFeePerDay,
    protectionPlanFeePerDay: DEFAULT_DISCOUNT_SETTINGS.protectionPlanFeePerDay,
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setDiscountForm({
        ...
        depositAmount: Number(nextDiscountSettings.depositAmount || DEFAULT_DISCOUNT_SETTINGS.depositAmount),
        taxPercentage: Number(nextDiscountSettings.taxPercentage || DEFAULT_DISCOUNT_SETTINGS.taxPercentage),
        servicePlatformFeePerDay: Number(nextDiscountSettings.servicePlatformFeePerDay || DEFAULT_DISCOUNT_SETTINGS.servicePlatformFeePerDay),
        protectionPlanFeePerDay: Number(nextDiscountSettings.protectionPlanFeePerDay || DEFAULT_DISCOUNT_SETTINGS.protectionPlanFeePerDay),
  ...
  const handleDiscountSave = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setDiscountForm({
        ...
        depositAmount: Number(nextSettings.depositAmount || DEFAULT_DISCOUNT_SETTINGS.depositAmount),
        taxPercentage: Number(nextSettings.taxPercentage || DEFAULT_DISCOUNT_SETTINGS.taxPercentage),
        servicePlatformFeePerDay: Number(nextSettings.servicePlatformFeePerDay || DEFAULT_DISCOUNT_SETTINGS.servicePlatformFeePerDay),
        protectionPlanFeePerDay: Number(nextSettings.protectionPlanFeePerDay || DEFAULT_DISCOUNT_SETTINGS.protectionPlanFeePerDay),
    ...
    <AppShell>
      ...
      <div className="mt-8 grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(0,0.9fr)]">
        <section className="glass-card rounded-2xl border-2 border-amber-300/60 p-6 shadow-[0_18px_40px_-28px_rgba(245,158,11,0.65)]">
          ...
          <form onSubmit={handleDiscountSave} className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3 xl:grid-cols-4">
            ...
            <label className="block">
              <span className="mb-2 block text-sm font-medium text-zinc-700">Global deposit amount</span>
              <input
                type="number"
                min="0"
                step="0.01"
                name="depositAmount"
                value={discountForm.depositAmount}
                onChange={handleDiscountChange}
                className="w-full rounded-xl border border-zinc-300 bg-white px-4 py-3 text-zinc-900 outline-none ring-0 transition focus:border-zinc-500"
              />
              <p className="mt-2 text-xs text-zinc-500">
                Refundable hold applied to all reservations in staff and public booking flows.
              </p>
            </label>

            <label className="block">
              <span className="mb-2 block text-sm font-medium text-zinc-700">Tax percentage</span>
              <input
                type="number"
                min="0"
                max="100"
                step="0.01"
                name="taxPercentage"
                value={discountForm.taxPercentage}
                onChange={handleDiscountChange}
                className="w-full rounded-xl border border-zinc-300 bg-white px-4 py-3 text-zinc-900 outline-none ring-0 transition focus:border-zinc-500"
              />
            </label>
            ...
            <label className="block">
              <span className="mb-2 block text-sm font-medium text-zinc-700">Service/Platform fee (per day)</span>
              <input
                type="number"
                min="0"
                step="0.01"
                name="servicePlatformFeePerDay"
                value={discountForm.servicePlatformFeePerDay}
                onChange={handleDiscountChange}
                className="w-full rounded-xl border border-zinc-300 bg-white px-4 py-3 text-zinc-900 outline-none ring-0 transition focus:border-zinc-500"
              />
            </label>
            ...
            <label className="block">
              <span className="mb-2 block text-sm font-medium text-zinc-700">Protection plan (USD per day)</span>
              <div className="flex items-center rounded-xl border border-zinc-300 bg-white px-4 py-3 focus-within:border-zinc-500">
                <span className="mr-2 text-zinc-500">$</span>
                <input
                  type="number"
                  min="0"
                  step="0.01"
                  name="protectionPlanFeePerDay"
                  value={discountForm.protectionPlanFeePerDay}
                  onChange={handleDiscountChange}
                  className="w-full bg-transparent text-zinc-900 outline-none"
                />
              </div>
              <p className="mt-2 text-xs text-zinc-500">Enter the daily protection plan fee in dollars.</p>
            </label>

            <div className="md:col-span-3 xl:col-span-4 flex items-center justify-between gap-4 pt-2">
              <p className="text-sm text-zinc-500">
                Set a tier to 0 to disable it without removing lower-tier discounts. Tax and per-day fees apply globally to all reservations.
              </p>
              <button
                type="submit"
        ...
        <div className="space-y-6">
          ...
          <section className="glass-card rounded-2xl p-6">
            ...
            <div className="mt-4 rounded-xl border border-zinc-200 bg-white/70 px-4 py-4">
              <p className="text-sm uppercase tracking-[0.2em] text-zinc-500">Global deposit</p>
              <p className="mt-1 text-lg font-semibold text-zinc-900">${Number(discountSettings.depositAmount || 0).toFixed(2)}</p>
            </div>

            <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
              <div className="rounded-xl border border-zinc-200 bg-white/70 px-4 py-4">
                <p className="text-sm uppercase tracking-[0.2em] text-zinc-500">Tax</p>
                <p className="mt-1 text-lg font-semibold text-zinc-900">{Number(discountSettings.taxPercentage || 0).toFixed(2)}%</p>
              </div>
              <div className="rounded-xl border border-zinc-200 bg-white/70 px-4 py-4">
                <p className="text-sm uppercase tracking-[0.2em] text-zinc-500">Service/Platform Fee</p>
                <p className="mt-1 text-lg font-semibold text-zinc-900">${Number(discountSettings.servicePlatformFeePerDay || 0).toFixed(2)}/day</p>
              </div>
              <div className="rounded-xl border border-zinc-200 bg-white/70 px-4 py-4">
                <p className="text-sm uppercase tracking-[0.2em] text-zinc-500">Protection Plan</p>
                <p className="mt-1 text-lg font-semibold text-zinc-900">${Number(discountSettings.protectionPlanFeePerDay || 0).toFixed(2)}/day</p>
              </div>
            </div>
