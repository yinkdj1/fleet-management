...
function CarDiagram({
  ...
  return (
    <div className="space-y-2">
      <p className="text-sm text-gray-600">Click on the diagram to mark a damage location.</p>
...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    api.get(`/bookings/${bookingId}`)
      ...
      .catch(() => {/* summary failure is non-blocking */})
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!mileageOut || Number(mileageOut) <= 0) {
      setError("A valid mileage out is required.");
      return;
    }
    ...
    <AppShell>
      ...
      {!loadingBooking && booking && (
        <div className="mb-6 rounded-xl border border-blue-100 bg-blue-50 p-4">
          <p className="mb-2 text-sm font-semibold text-blue-900">Booking {formatBookingId(booking.id)}</p>
          <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm text-blue-800 md:grid-cols-4">
            ...
            <p><span className="font-medium">Return:</span> {new Date(booking.returnDatetime).toLocaleString()}</p>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-8 max-w-3xl">
        ...
        {/* Section 1 — Mileage & fuel */}
        <section className="rounded-xl border bg-white p-5 shadow-sm">
          <h2 className="mb-4 text-lg font-semibold">Drop-Off Details</h2>
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">Mileage Out <span className="text-red-500">*</span></label>
              <input
                type="number"
                min="0"
                step="1"
            ...
            <div className="sm:col-span-2">
              ...
              <textarea
                rows={3}
                placeholder="Any notes about the vehicle condition at drop-off…"
                value={notesOut}
        ...
        <section className="rounded-xl border bg-white p-5 shadow-sm">
          ...
          <p className="mb-4 text-sm text-gray-500">Click the vehicle diagram to mark any pre-existing or new damage. Each marker will be saved with your description.</p>
          <CarDiagram
