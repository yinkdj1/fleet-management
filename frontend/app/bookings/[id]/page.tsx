...
export default function BookingDetailPage() {
  ...
  useEffect(() => {
    ...
    setMinPickupDatetime(`${year}-${month}-${day}T${hours}:${minutes}`);
  }, []);

  // Auto-open edit modal when ?edit=true is in the URL
  useEffect(() => {
    if (booking && searchParams.get("edit") === "true") {
      openEditModal();
    }
  ...
  const closeEditModal = () => {
    if (savingEdit || cancellingBooking) return;
    setIsEditOpen(false);
    setEditError("");
    setEditVehicleUnavailable(false);
  };
  ...
  const cancelReservation = async () => {
    ...
    try {
      ...
      setCancellationMessage(
        `Booking ${formatBookingId(booking.id)} has been cancelled. The vehicle is now available for booking.`
      );
      await fetchBooking();
    } catch (err: any) {
      setEditError(err.response?.data?.message || "Failed to cancel booking.");
    } finally {
  ...
  const saveEditBooking = async () => {
    ...
    try {
      ...
      const isAvailabilityError =
        ...
        (errorsObj.vehicleId || "").toLowerCase().includes("overlapping");
      if (isAvailabilityError) {
        setEditVehicleUnavailable(true);
        setEditError("");
      } else {
    ...
    <AppShell>
      ...
      {cancellationMessage && (
        <div className="mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3">
          <p className="font-semibold text-red-700">{cancellationMessage}</p>
        </div>
      )}
      ...
      {isEditOpen && booking && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4">
          <div className="w-full max-w-lg rounded-xl bg-white p-5 shadow-2xl">
            ...
            <div className="mt-4 space-y-3">
              <div>
                ...
                <input
                  ...
                  min={minPickupDatetime || undefined}
                  onChange={(e) => {
                    setEditPickupDatetime(e.target.value);
                    setEditVehicleUnavailable(false);
                    setEditError("");
                  }}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    setEditReturnDatetime(e.target.value);
                    setEditVehicleUnavailable(false);
                    setEditError("");
                  }}
                  className="w-full rounded border px-3 py-2"
            ...
            {editVehicleUnavailable && (
              <div className="mt-3 rounded-lg border border-red-300 bg-red-50 px-4 py-3">
                <p className="font-bold text-red-700">
                  The vehicle is not available for the selected dates. Please choose different dates or cancel this reservation.
                </p>
              </div>
            )}
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button
                ...
                disabled={savingEdit || cancellingBooking}
              ...
              {editVehicleUnavailable && (
                <button
                  ...
                  disabled={cancellingBooking || savingEdit}
                  className="rounded bg-red-600 px-4 py-2 text-sm text-white disabled:opacity-60"
                >
                  {cancellingBooking ? "Cancelling..." : "Cancel Reservation"}
                </button>
              )}
              <button
                type="button"
